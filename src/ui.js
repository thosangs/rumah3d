...
export function renderHitungan(R, el, onPick) {
  ...
  <p class="muted">Plin ditampilkan di 3D sebagai strip 10 cm di kaki tembok ruang dalam (tidak di kamar mandi karena dindingnya sudah granit; tidak di teras). Belah 8 strip/keping berarti tiap strip ±9,7 cm (dikurangi mata potong). Kalau mau tepat 10 cm, hanya 7 strip/keping → ${Math.ceil(R.plinthTotal.strips / 7)} keping.</p>`;
  ...
  h += `<h3>Dinding kamar mandi — granit 30×60 dipasang tegak (30 lebar × 60 tinggi), full dari lantai ${cfg.wallZones.topH == null ? 'sampai bawah dak (lt.1 3,68 m, lt.2 3,35 m — toilet tidak berplafon di model; baris teratas dipotong menyesuaikan)' : `sampai plafon ${r1(cfg.wallZones.bottomH + cfg.wallZones.topH)} m`}: bawah motif teraso titik biru ${cfg.wallZones.bottomH == null ? 'beda tiap KM sebagai pembanding — Lt.1 180 cm, Lt.2 kiri 120 cm, Lt.2 kanan 60 cm' : Math.round(cfg.wallZones.bottomH * 100) + ' cm'}, sisanya putih polos${cfg.wallZones.topH == null ? '' : ` ${Math.round(cfg.wallZones.topH * 100)} cm`}</h3>`;
  ...
  for (const b of R.baths) {
    h += `<tr class="total"><td>${b.name} <span class="badge">keliling ${b.perimeter} m · tinggi ${r1(b.zones.bottomH + b.zones.topH)} m · teraso ${Math.round(b.zones.bottomH * 100)} cm</span></td><td class="num">${b.areaBawah + b.areaAtas} m²</td><td class="num">${b.bawah.total} keping<br/><span class="cuts">${b.bawah.full} utuh + ${b.bawah.cuts} potongan</span></td><td class="num">${b.atas.total} keping<br/><span class="cuts">${b.atas.full} utuh + ${b.atas.cuts} potongan</span></td></tr>`;
  ...
  <p class="muted">Teraso 180 cm = 3 baris × 60 cm tegak, pas tanpa potongan mendatar. Posisi pintu (kusen 70 cm, tinggi 190/195 cm, menempel sudut) dan jendela (60×40, ambang 2,18 m, di tengah dinding luar) diukur dari model SketchUp; bukaan keramik mengikuti posisi itu. Kalau pintu riil 210 cm, selisihnya ±0,15 m² per pintu (≈1 keping). Tinggi keramik default sampai bawah dak; kalau nanti dipasang plafon, ganti di Setelan.</p>`;
  ...
  h += row(`Dinding KM bawah · ${cfg.wallZones.bottomH == null ? '180 / 120 / 60 cm' : Math.round(cfg.wallZones.bottomH * 100) + ' cm'} (tukang: 100 cm)`, T.kmBawah, Ri.wall.bawah.area, Ri.wall.bawah.pieces, Ri.wall.bawah.boxesNoWaste, Ri.wall.bawah.boxes, 'Tukang pakai keliling 10 m/KM; denah 5,9 m/KM. Motif teraso.');
  h += row(`Dinding KM atas · ${cfg.wallZones.topH == null ? 'sisa sampai dak' : Math.round(cfg.wallZones.topH * 100) + ' cm'} (tukang: 140 cm)`, T.kmAtas, Ri.wall.atas.area, Ri.wall.atas.pieces, Ri.wall.atas.boxesNoWaste, Ri.wall.atas.boxes, 'Putih polos, dari batas teraso sampai atas.');
