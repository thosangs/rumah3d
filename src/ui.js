...
export function renderHitungan(R, el, onPick) {
  ...
  h += `<h3>Dinding kamar mandi — granit 30×60 dipasang tegak (30 lebar × 60 tinggi), full sampai plafon ${r1(cfg.wallZones.bottomH + cfg.wallZones.topH)} m: bawah motif teraso ${Math.round(cfg.wallZones.bottomH * 100)} cm, sisanya putih polos ${Math.round(cfg.wallZones.topH * 100)} cm</h3>`;
  ...
  <p class="muted">Teraso 180 cm = 3 baris × 60 cm tegak, pas tanpa potongan mendatar. Di atas pintu (210 cm) tersisa potongan 30 cm dari baris putih pertama. Jendela toilet 60×40 cm (ambang 2,18 m, puncak 2,58 m) diukur dari model SketchUp; bukaan keramik mengikuti posisi itu. Plafon diasumsikan 2,7 m — ubah di Setelan kalau berbeda.</p>`;
