...
export function renderHitungan(R, el, onPick) {
  ...
  h += `<h3>Dinding kamar mandi — granit 30×60 dipasang tegak (30 lebar × 60 tinggi), full dari lantai ${cfg.wallZones.topH == null ? 'sampai bawah dak (lt.1 3,68 m, lt.2 3,35 m — toilet tidak berplafon di model; baris teratas dipotong menyesuaikan)' : `sampai plafon ${r1(cfg.wallZones.bottomH + cfg.wallZones.topH)} m`}: bawah motif teraso titik biru ${Math.round(cfg.wallZones.bottomH * 100)} cm, sisanya putih polos${cfg.wallZones.topH == null ? '' : ` ${Math.round(cfg.wallZones.topH * 100)} cm`}</h3>`;
  ...
  for (const b of R.baths) {
    h += `<tr class="total"><td>${b.name} <span class="badge">keliling ${b.perimeter} m · tinggi ${r1(b.zones.bottomH + b.zones.topH)} m</span></td><td class="num">${b.areaBawah + b.areaAtas} m²</td><td class="num">${b.bawah.total} keping<br/><span class="cuts">${b.bawah.full} utuh + ${b.bawah.cuts} potongan</span></td><td class="num">${b.atas.total} keping<br/><span class="cuts">${b.atas.full} utuh + ${b.atas.cuts} potongan</span></td></tr>`;
  ...
  h += row(`Dinding KM atas · ${cfg.wallZones.topH == null ? '180 cm sampai dak' : Math.round(cfg.wallZones.topH * 100) + ' cm'} (tukang: 140 cm)`, T.kmAtas, Ri.wall.atas.area, Ri.wall.atas.pieces, Ri.wall.atas.boxesNoWaste, Ri.wall.atas.boxes, 'Putih polos, dari 180 cm sampai atas.');
