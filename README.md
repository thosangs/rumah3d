...
- `src/furniture.js` — memuat aset furnitur GLB sesuai `src/layout/<ruang>.js` (posisi/rotasi per ruang) + elemen parametrik (railing oval tangga/void, lemari bawah tangga, downlight). `src/assets.js` = manifest aset: `bl_*` dibuat di Blender (`scripts/blender/build_furniture.py`), `ph_*` Poly Haven CC0, `fm_*` FurniMesh library — sumber & lisensi di `models/furniture/SOURCES.md`. Penempatan mengikuti bukaan nyata di GLB (pintu utama di tembok x=6,5 dari teras depan, jendela r. keluarga hanya di tembok depan, tembok kanan polos, jendela KT1 & KT2 di tembok kiri, pintu geser balkon KTU di tembok kiri).
- `scripts/shot.mjs` / `scripts/shots.mjs` + `scripts/views.json` — screenshot headless (Chrome) per sudut pandang ruang (`?shot=1&cam=&look=` di app); `scripts/sheet.py` menggabungkan PNG jadi lembar kontak. Dipakai untuk loop review multi-agen: potret → bandingkan dengan render `drawings/hal-21..40.jpg` → edit layout → potret ulang.
